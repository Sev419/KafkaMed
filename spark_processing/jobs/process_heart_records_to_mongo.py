"""KafkaMed Spark Structured Streaming consumer that persists to MongoDB."""
...
from spark_processing.src.mongo_config import load_mongo_config  # noqa: E402
from spark_processing.src.mongo_writer import write_microbatch_to_mongo  # noqa: E402
...
LOGGER = logging.getLogger("kafkamed.spark_heart_mongo")


def setup_logger() -> None:
    """Configure console logging for the persistence job."""
...
def build_stream():
    ...
    LOGGER.info(
        "Configuracion Mongo: uri=%s db=%s collection=%s",
        mongo_config.uri,
    ...
    spark = create_spark_session(
        components,
        LOGGER,
        app_name="KafkaMed_HeartRecords_To_Mongo",
    ...
    return spark, parsed_stream, kafka_config, mongo_config


def run() -> int:
    """Run the Spark job and persist records to MongoDB."""
    setup_logger()
    start = time.time()
    spark = None
    query = None
    ...
    try:
        LOGGER.info("Inicio del consumidor Spark Structured Streaming a MongoDB.")
        ...
        spark, parsed_stream, kafka_config, mongo_config = build_stream()
        ...
        writer = (
            parsed_stream.writeStream.foreachBatch(
                lambda batch_df, batch_id: write_microbatch_to_mongo(
                    batch_df,
                    batch_id,
                    mongo_config,
                    LOGGER,
            ...
            .queryName("kafkamed_heart_records_mongo")
        )
        ...
        LOGGER.info("Consumidor Spark -> Mongo finalizado en %.2f segundos.", time.time() - start)
        return 0
    except Exception:
        LOGGER.exception("El consumidor Spark -> Mongo fallo.")
        return 1
    finally:
